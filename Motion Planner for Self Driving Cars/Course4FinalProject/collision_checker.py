    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                circle_locations[:, 0] = (circle_offsets * np.cos(path[2][j]) ) + path[0][j]
                circle_locations[:, 1] = (circle_offsets * np.sin(path[2][j]) ) + path[1][j]
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # --------------------------------------------------------------
                r = len(paths[i][0])
                score = ( (goal_state[0] - paths[i][0][r-1])**2 + (goal_state[1] - paths[i][1][r-1])**2 )**0.5
                # --------------------------------------------------------------
